...
feature {NONE} -- Initialization
	...
	add_selection_actions
			-- Subscribe station views to left click.
		require
			map_exists: map /= Void
		local
			v: STATION_VIEW
		do
			...
			loop
				v := map.station_view (i.item)
				v.on_left_click_no_args.extend_back (agent unhighlight_stations)
				v.on_left_click_no_args.extend_back (agent exercise.highlight_short_distance (v.model))
			end
		end

feature {NONE} -- Implementation

	exercise: SHORT_TRIPS
